...
- THE PHOTO MUST SHOW THE NAMED PERSON: a story about player X must use a photo of player X himself — a photo of a DIFFERENT player in the same club kit is WRONG. The Commons file NAME or file DESCRIPTION must explicitly name the player; generic "Ahly player" identification is NOT acceptable. If no free photo of the named player exists, fall back IN THIS ORDER: (1) the named CLUB's stadium or a clearly club-identified scene (tifo/banner) — the club must be one actually named in the story, (2) the branded placeholder `https://yallascore.site/media/ph-pitch.svg` with an empty image_credit (no download needed, the file is already deployed). There is NO "any Egyptian stadium" fallback. Write the credit line for what the photo ACTUALLY shows. Never guess identity from kit colours alone, and never pick between same-name clubs (لييج/ستاندرد لييج، الأهلي مصر/جدة) by guessing — if the story doesn't disambiguate, that's a placeholder case.
